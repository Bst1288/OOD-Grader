...
class TorKham:
	...
	def play(self, word):
		...
		if self.temp[len(self.temp)-2].lower() == word[0].lower() and self.temp[len(self.temp)-1].lower() == word[1].lower():
			return str(self.words)
...
for symbol in S:
    if symbol[0] == "X":
        ...
        char,temp = symbol.split()
        print("'" + temp + "'" + " -> " + torkham.play(temp))
    else:
        print("'" + symbol + "'" + " is Invalid Input !!!")
        break