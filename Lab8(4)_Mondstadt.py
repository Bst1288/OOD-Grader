        ...
        if self.root == None:
            ...
            if self.num+1 > max_node:
                ...
                if self.num+1 <= max_node-((max_node-(pow(2,h)-1))/2):
                    insertSub(current.left,self.num - round(pow(2,h)/2),val,freq)
                else:
                    insertSub(current.right,self.num - pow(2,h),val,freq)
                self.num+=1

def insertSub(r,num,val,freq):
    if r != None:
        h = height(r)
        max_node = pow(2,h+1)-1
        current = r
        ...
        if num+1 <= max_node-((max_node-(pow(2,h)-1))/2):
            insertSub(current.left,num - round(pow(2,h)/2),val,freq)
        else:
            insertSub(current.right,num - pow(2,h),val,freq)
    else:
        return
