...
for i in l:
    ...
    if number[0] == 'A':
        ...
        dls.insert(dls.nodeAt(0),number[1])
    elif number[0] == 'I':
        index,data = number[1].split(":")
        if int(index) < 0 or len(dls) < int(index):
            print("Data cannot be added")
        else:
            dls.insert(dls.nodeAt(int(index)),data)
            print("index = {0} and data = {1}".format(index,data))
    elif number[0] == 'R':
        idx = dls.indexOf(number[1])
        r = dls.remove(number[1])
        if r==-1:
            print("Not Found!")
        else:
            print("removed : {0} from index : {1}".format(str(r.data),str(idx)))
    print("linked list : " + str(dls))
    print("reverse : " + dls.str_reverse())